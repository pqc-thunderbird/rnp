...
TEST_F(rnp_tests, hkdf_test_case_1)
{
    ...
    */
    ...
    rnp::Hkdf_Botan hkdf(PGP_HASH_SHA256);

    Botan::secure_vector<uint8_t> PRK = hkdf.extract(salt, IKM);
    assert_int_equal(PRK.size(), PRK_expected.size());
    assert_memory_equal(PRK.data(), PRK_expected.data(), PRK.size());

    std::vector<uint8_t> OKM = hkdf.expand(PRK, info, L);
    assert_int_equal(OKM.size(), OKM_expected.size());
    assert_memory_equal(OKM.data(), OKM_expected.data(), OKM.size());

    uint8_t OKM2[L];
    hkdf.extract_expand(salt.data(), salt.size(), IKM.data(), IKM.size(), info.data(), info.size(), OKM2, L);
    assert_memory_equal(OKM.data(), OKM2, OKM.size());
}
